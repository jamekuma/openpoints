...
from .pointnet2_utils import farthest_point_sample, index_points, query_knn_point, query_ball_point
...
def sample_and_group(npoint, radius, nsample, center, normal, feature, return_normal=True, return_polar=False, cuda=False):
    ...
    """
    assert (normal is None) ^ return_normal    # when return_normal == True, normal must be not None
    # sample
    fps_idx = farthest_point_sample(center[..., :3], npoint, cuda=cuda)  # [B, npoint, A]
    torch.cuda.empty_cache()
    # sample center
    new_center = index_points(center, fps_idx, cuda=cuda, is_group=False)
    torch.cuda.empty_cache()
    # sample normal
    new_normal = index_points(normal, fps_idx, cuda=cuda, is_group=False) if normal is not None else None 
    torch.cuda.empty_cache()

    # group
    idx = query_ball_point(radius, nsample, center[..., :3], new_center[..., :3], cuda=cuda)
    torch.cuda.empty_cache()
    # group normal
    group_normal = index_points(normal, idx, cuda=cuda, is_group=True) if normal is not None else None   # [B, npoint, nsample, B]
    torch.cuda.empty_cache()
    # group center
    group_center = index_points(center, idx, cuda=cuda, is_group=True)  # [B, npoint, nsample, A]
    torch.cuda.empty_cache()
    group_center_norm = group_center
    group_center_norm[..., :3] = group_center_norm[..., :3] - new_center[..., :3].unsqueeze(2)
    torch.cuda.empty_cache()

    # group polar
    if return_polar:
        group_polar = xyz2sphere(group_center_norm[..., :3])
    ...
    if feature is not None:
        ...
        new_feature = torch.cat([group_center_norm, group_normal], dim=-1) if return_normal \
...
def sample_and_group_all(center, normal, feature, return_normal=True, return_polar=False):
    ...
    group_center = center.view(B, 1, N, CC)
    if return_polar:
...
class SurfaceAbstractionCD(nn.Module):
    ...
    def __init__(self, npoint, radius, nsample, feat_channel, pos_channel, mlp, group_all,
        ...
        self.mlp_f0 = nn.Conv2d(feat_channel, mlp[0], 1) if feat_channel != 0 else None
    ...
    def forward(self, center, normal, feature):
        ...
        if feature is not None:
            feature = feature.permute(0, 2, 1)
        assert (normal is None) ^ self.return_normal    # when self.return_normal == True, normal must be not None
        if self.group_all:
            new_center, new_normal, new_feature = sample_and_group_all(center, normal, feature,
                                                                       return_normal=self.return_normal,
                                                                       return_polar=self.return_polar)
        ...
        feat = self.bn_f0(self.mlp_f0(new_feature[:, self.pos_channel:])) if new_feature.shape[1] - self.pos_channel > 0\
            else 0
        ...
        new_center = new_center.permute(0, 2, 1)
        new_normal = new_normal.permute(0, 2, 1) if new_normal is not None else None
...
class UmbrellaSurfaceConstructor(nn.Module):
    ...
    def forward(self, center):
        ...
        # normal
        group_normal = cal_normal(group_xyz, random_inv=self.random_inv, is_group=True) # [B, N, K-1, 3 (norm coord.)]
        # coordinate
        ...
        if self.return_dist:
            ...
            new_feature = torch.cat([group_center, group_polar, group_normal, group_pos], dim=-1)  # N+P+CP: 3+3+3+1=10
        else:
